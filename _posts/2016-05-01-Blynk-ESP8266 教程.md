...
- 好了， 可以把程序上传到 ESP8266
-- 成功的话 会出现： “构建选项已变更，全部重新构建

项目使用了 239,423 字节，占用了 (22%) 程序存储空间。最大为 1,044,464 字节。
全局变量使用了32,908字节，(40%)的动态内存，余留49,012字节局部变量。最大为81,920字节。
warning: espcomm_send_command: wrong direction/command: 0x01 0x07, expected 0x01 0x08
Uploading 243568 bytes from to flash at 0x00000000”
5. 远程控制 ESP8266
- 手机要连到与esp8266一样路由机上
